...
from utils_batch_RMS import get_results, get_eval_results, get_split_latents, split_do_tsne, plot_latents, CustomLoader, fit_knn_fn, fit_etc_fn, plot_psd
...
if __name__ == '__main__':
    ...
    with tables.open_file(config.DATA_FILENAME, mode="r") as pytables_file:
        ...
        # with tqdm(range(2), unit_scale=EFFECTIVE_BATCH_SIZE, disable=not USE_TQDM, file=sys.stdout) as pbar:
        ...
        for i, which in enumerate(['subject', 'task']):
            ...
            if which == 'subject':
                ...
                display_labels = [f'{s}' for s in test_loader.unique_subjects]
            elif which == 'task':
