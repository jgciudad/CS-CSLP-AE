...
def get_conversion_results(model, loader, subject_latents, task_latents, target_subject, target_task, channel, n, axes=None, plot_fcn=None):
    ...
    ss_mse = F.mse_loss(recon_erp_ss1[channel], real_erp1[channel]) if recon_erp_ss1 is not None else None
    sd_mse = F.mse_loss(recon_erp_sd1[channel], real_erp1[channel]) if recon_erp_sd1 is not None else None
...
def get_full_conversion_results(model, test_loader, subject_latents, task_latents, N, plot, figure_results={}):
    ...
    task_to_channel = {1: 'Fpz-Cz', 2: 'Fpz-Cz', 3: 'Fpz-Cz', 4: 'Fpz-Cz', 5: 'Fpz-Cz'}
