...
from data.preprocessing_utils import ButterworthFilter, MultitaperPreprocessor, WelchPreprocessor, center_windowing

class Sleepedf4Preprocessor(BasePreprocessor):
    ...
    def process_recording(self, signal_path, labels_path) -> Tuple[Dict[str, np.ndarray], list]:
        ...
        for new_c_name, old_c_name in self.dataset_config['channels'].items():
            ...
            X, new_labels = center_windowing(self.config,
                                 X,
                                 labels,
                                 30,
                                 4)
