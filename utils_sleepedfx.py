...
class CustomLoader():
    ...
    def __init__(self, data_dict, fold, dev_first_n=2, split='train'):
        self.split = split
        self.fold = fold
        ...
        self.data_mean = data_dict['fold_info'][fold]['mean'].detach().clone().contiguous().to(device)
        self.data_std = data_dict['fold_info'][fold]['std'].detach().clone().contiguous().to(device)
        
        if split == 'dev':
            self.unique_subjects = np.unique(self.subjects[data_dict['fold_info'][fold]['train_idx']])[:dev_first_n]
        elif split == 'test':
            self.unique_subjects = np.unique(self.subjects[data_dict['fold_info'][fold]['test_idx']])
        elif split == 'train':
            self.unique_subjects = np.unique(self.subjects[data_dict['fold_info'][fold]['train_idx']])[dev_first_n:]
        else:
        ...
        self.unique_tasks = np.unique(self.tasks)
...
def plot_spectrogram(ax, spectrogram, fs=128, time_length=30, title='Spectrogram', colorbar=False):
    ax.imshow(spectrogram, aspect='auto', origin='lower', cmap='magma')
    ax.set_xlabel('Time (s)')
    ax.set_ylabel('Frequency (Hz)')
    ax.set_title(title)
    ax.set_xticks(np.linspace(0, spectrogram.shape[1], 6))
    ax.set_xticklabels(np.linspace(0, time_length, 6, dtype=int))
    ax.set_yticks(np.linspace(0, spectrogram.shape[0], 8))
    ax.set_yticklabels(np.linspace(0, fs/2, 8, dtype=int))
    if colorbar:
