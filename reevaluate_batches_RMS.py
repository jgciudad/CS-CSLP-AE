import sys
import wandb
from split_model_RMS import SplitLatentModel
import torch
import numpy as np
from utils_batch_RMS import get_results, get_split_latents, split_do_tsne, plot_latents, CustomLoader, fit_knn_fn, fit_etc_fn, plot_psd
...
parser.add_argument('--data_experiment', type=str, default=None) # experiment to use for data loading, if i want to test a model with different data than from training
...
if __name__ == '__main__':
    ...
    target_runs = [run for run in runs if config.RUN_NAME in run.name]

    if len(target_runs) == 1:
        print(f"Found run: {target_runs[0].name} with ID: {target_runs[0].id}")
        # Now you can access the run using its ID
        run = api.run(f"{args.wandb_directory}/{target_runs[0].id}")
    elif len(target_runs) > 1 and args.id == '':
        print(f"Multiple runs found with the name {config.RUN_NAME}. Please specify the run ID.")
        for i, run in enumerate(target_runs):
            print(f"{i}: {run.name} with ID: {run.id}")
            
    elif len(target_runs) > 1 and args.id != '':
        print(f"Multiple runs found with the name {config.RUN_NAME}. Using the specified ID: {args.id}")
        run = api.run(f"{args.wandb_directory}/{args.id}")
    else:
    ...
    if args.data_experiment is not None:
        data_config = ConfigLoader(fold=FOLD, run_name='timestamp', experiment=args.data_experiment)
    else:
        data_config = config
                
    with tables.open_file(data_config.DATA_FILENAME, mode="r") as pytables_file:
        ...
        with torch.no_grad():
            loader = CustomLoader(data_config, pytables_file, subjects=subjects, split='dev', subsample_size=subsample_size)
        ...
        with torch.inference_mode(True):
            ...
            if args.conversion_results:
                figure_results, mse_results = get_full_conversion_results(model, loader, subject_latents, task_latents, args.conversion_N, True, IN_CHANNELS, args.conversion_channel, config.CHANNELS.index(args.conversion_channel), args.split)
            ...
            plt.tight_layout()
            figure_results[f'results/{args.split}/recon'] = wandb.Image(fig)
            plt.close(fig)
            ...
            ax[1].legend()
            plt.tight_layout()
            figure_results[f'results/{args.split}/pca'] = wandb.Image(fig)
            plt.close(fig)
            ...
            plt.tight_layout()
            figure_results[f'results/{args.split}/tsne'] = wandb.Image(fig)
            plt.close(fig)
            ...
            for i, which in enumerate(['subject', 'task']):
                ...
                if which == 'subject':
                    acc = test_results['XGB/' + args.split + '/subject/balanced_accuracy']
                elif which == 'task':
                    acc = test_results['XGB/' + args.split + '/task/balanced_accuracy']
                ax[i].set_title(f'{which.capitalize()}\nBalanced Accuracy: {100*acc:.2f}%', fontsize=12)
            plt.tight_layout()
            figure_results[f'results/{args.split}/cm_fig'] = wandb.Image(fig)
            plt.close(fig)
            plt.close('all')
