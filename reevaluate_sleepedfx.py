...
import torch
import numpy as np
from utils_sleepedfx import get_results, get_split_latents, split_do_tsne, CustomLoader
...
if __name__ == '__main__':
    ...
    for run in tqdm(runs):
        ...
        model = SplitLatentModel(IN_CHANNELS, CHANNELS, LATENT_DIM, NUM_LAYERS, KERNEL_SIZE, recon_type=RECON_TYPE, time_resolution=240)
        ...
        with torch.inference_mode(True):
            ...
            # indices = np.random.choice(len(subject_latents), size=int(len(subject_latents) * 0.1), replace=False) # TODO remove this line
            # subject_latents = subject_latents[indices]
            # task_latents = task_latents[indices]
