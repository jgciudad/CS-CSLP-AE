...
class BaselineModel(nn.Module):
    ...
    def __init__(self, in_channels, channels, latent_dim, num_layers, kernel_size, recon_type="mse", content_cosine=False, time_resolution=256, losses=['recon']):
        ...
        self.rms_channels = len(in_channels)
        self.in_channels = len([c for c in in_channels if 'EEG' in c])
        self.channels = channels
        ...
        self.encoder_bottleneck = TransformerBottleneck(channels + self.rms_channels, num_layers, self.latent_seqs)
    ...
    def get_x(self, x):
        _, x1, s1, t1, r1, rms1 = self.loader.sample_batch(self.batch_size)
        x['x'] = x1.to(device)
        x['x_rms'] = rms1.to(device)
    ...
    def get_x_prop(self, x, prop='s'):
        if 'x_p_' + prop + '1' not in x or 'x_p_' + prop + '2' not in x:
            _, x1, s1, t1, r1, rms1 = self.loader.sample_by_property(prop)
            _, x2, s2, t2, r2, rms2 = self.loader.sample_by_property(prop)
            ...
            if self.loader.split not in ['test', 'dev', 'eval']:
                for i in range(0, self.batch_size // (2*sample_size) - 1):
                    _, x1, s1, t1, r1, rms1 = self.loader.sample_by_property(prop)
                    samples1.append(x1)
                    rms_samples1.append(rms1)
                    #s_s1.append(s1)
                    #t_s1.append(t1)
                    #r_s1.append(r1)
                    _, x2, s2, t2, r2, rms2 = self.loader.sample_by_property(prop)
                    samples2.append(x2)
                    rms_samples2.append(rms2)
                    #s_s2.append(s2)
                    #t_s2.append(t2)
                    #r_s2.append(r2)
            samples1 = torch.stack(samples1, 0)
            samples2 = torch.stack(samples2, 0)
            rms_samples1 = torch.stack(rms_samples1, 0)
            rms_samples2 = torch.stack(rms_samples2, 0)
            ...
            x['x_rms_p_' + prop + '1'] = rms_samples1.to(device)
            x['x_rms_p_' + prop + '2'] = rms_samples2.to(device)
        return x
...
class SplitLatentModel(BaselineModel):
    ...
    def __init__(self, in_channels, channels, latent_dim, num_layers, kernel_size, time_resolution=256, recon_type=True, content_cosine=False):
        ...
        self.encoder_out_net = ConvBlock(channels + self.rms_channels, channels + self.rms_channels, channels + self.rms_channels)
        
        self.subject_encoder_out_net = ConvBlock(channels + self.rms_channels, channels + self.rms_channels, latent_dim)
        
        self.subject_decoder_in_net = ConvBlock(latent_dim, channels, channels)
        
        self.task_encoder_out_net = ConvBlock(channels + self.rms_channels, channels + self.rms_channels, latent_dim)
    ...
    def pre_encode(self, x, x_rms):
        x = self.encoder_in(x)
        x = self.encoder(x)
        x = x.permute(0, 2, 1)
        x_rms = x_rms.reshape(-1, x_rms.shape[-1])
        x_rms = x_rms.unsqueeze(1).expand(-1, x.shape[1], -1) 
        x = torch.concat([x, x_rms], dim=-1)
    ...
    def subject_task_encode(self, x, x_rms):
        x = self.pre_encode(x, x_rms)
        x = self.encoder_out_net(x)
    ...
    def get_s_t(self, x):
        ...
        if 's' not in x or 't' not in x:
            s, t = self.subject_task_encode(x['x'], x['x_rms'])
            x['s'] = s
            x['t'] = t
        return x
    ...
    def collapsed_split_apply(self, x, x_rms, f, *args, **kwargs):
        x_shape = x.size()
        x = x.view(-1, *x_shape[2:])
        s, t = f(x, x_rms, *args, **kwargs)
    ...
    def get_s_t_prop(self, x, prop):
        ...
        if 's_p_' + prop + '1' not in x or 't_p_' + prop + '1' not in x or 's_p_' + prop + '2' not in x or 't_p_' + prop + '2' not in x:
            s1, t1 = self.collapsed_split_apply(x['x_p_' + prop + '1'], x['x_rms_p_' + prop + '1'], self.subject_task_encode) # obtain subject and task embeddings for x1
            s2, t2 = self.collapsed_split_apply(x['x_p_' + prop + '2'], x['x_rms_p_' + prop + '2'], self.subject_task_encode) # obtain subject and task embeddings for x1
            x['s_p_' + prop + '1'] = s1
            x['t_p_' + prop + '1'] = t1
            x['s_p_' + prop + '2'] = s2
            x['t_p_' + prop + '2'] = t2
