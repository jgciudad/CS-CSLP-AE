...
from utils_sleepedfx import get_results, get_eval_results, get_split_latents, split_do_tsne, plot_latents, CustomLoader, fit_knn_fn, fit_etc_fn, plot_spectrogram
from conversion_utils_sleepedf import get_full_conversion_results
...
parser.add_argument('--sampling_rate', type=int, default=128) # 160 kornum, 240 SleepEDFx (ideally 200, need to regenerate it)
...
parser.add_argument('--conversion_N', type=int, default=2000) # maybe i should reduce this (it means that we are generating 2000 samples using 2000 samples with specific conditions S.t, D.t etc, but there might not be such a large number and in that case the model is just upsamlpling until reaching 2000, see sample_latents function in conversion_utils_sleepedf.py)
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        data_dict["data"] = (data_dict["data"] - data_dict["fold_info"][fold]["mean"]) / data_dict["fold_info"][fold]["std"]
        loader = CustomLoader(data_dict, fold=fold, split='train')
        test_loader = CustomLoader(data_dict, fold=fold, split='test')
        eval_loader = CustomLoader(data_dict, fold=fold, split='dev')
        del data_dict
    ...
    n_samples = (5, 4)
    fig, axs = plt.subplots(n_samples[0], n_samples[1]*2, figsize=(40, 15))
    for i in range(n_samples[0] * n_samples[1]):
        mse_sample = torch.mean((x['x'][i] - x['x_hat'][i]) ** 2).item()
        plot_spectrogram(axs[i//n_samples[1], 2*(i%4)], x['x'][i].cpu().detach().numpy(), args.sampling_rate, 30, f"x, Sample {i}, Stage: {model.loader.task_to_label[x['T'][i].item()]}", colorbar=True)
        plot_spectrogram(axs[i//n_samples[1], 2*(i%4)+1], x['x_hat'][i].cpu().detach().numpy(), args.sampling_rate, 30, f"x_hat, S{i}, MSE: {mse_sample:.4f}, Stage: {model.loader.task_to_label[x['T'][i].item()]}", colorbar=True)
    plt.tight_layout()
